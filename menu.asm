...
statusTitle DB 13,10,13,10
            DB "========================================",13,10
            DB "         SEARCH INVOICE RECORD          ",13,10
            DB "========================================",13,10,"$"
...
foundBookingMsg DB 13,10,"[SUCCESS] Booking Invoice Record Found!",13,10,"$"
...
recordMenuMsg DB 13,10,13,10
              ...
              DB "2. Search Invoice Record                ",13,10
              DB "3. Back                                 ",13,10
              DB "----------------------------------------",13,10
...
; =========================================================
; Detailed Booking-Invoice lookup display (Search Invoice Record)
; =========================================================
detailedBookingHeader DB 13,10,13,10
                       DB "========================================",13,10
                       DB "         BOOKING INVOICE DETAILS         ",13,10
                       DB "========================================",13,10,"$"
detBkInvoiceLbl DB "Invoice ID   : INV$"
detBkBookingLbl DB 13,10,"Booking ID   : BK-$"
detBkSportLbl   DB 13,10,"Sport Type   : $"
detBkCourtLbl   DB 13,10,"Court        : Court $"
detBkTotalLbl   DB 13,10,"Total Amount : RM $"
detBkFooter     DB 13,10,"========================================",13,10,"$"
...
msgViewHeader DB 13,10,13,10
              DB "==================================================",13,10
              DB "         ADD-ON / EQUIPMENT INVOICE RECORDS       ",13,10
              DB "==================================================",13,10
              DB "INVOICE ID | CUST NAME        | COURT | TOTAL (RM)",13,10
              DB "--------------------------------------------------",13,10,"$"

msgBookingViewHeader DB 13,10,13,10
              DB "==================================================",13,10
              DB "            VENUE BOOKING INVOICE RECORDS         ",13,10
              DB "==================================================",13,10
              DB "INVOICE ID | SPORT            | COURT | TOTAL (RM)",13,10
              DB "--------------------------------------------------",13,10,"$"
...
msgQuantity DB 13,10
            DB "Enter quantity (1-99): $"

qtyInputBuf DB 4, 0, 4 DUP('$')
...
msgInvalidCourtID DB 13,10,"[ERROR] Invalid Court ID! Enter 1-9 digits only.",13,10,"$"
...
; =========================================================
; SEARCH & DISPLAY INVOICE RECORD BY ID
; Searches BOTH the add-on/equipment invoice table (dbTable) and
; the venue booking invoice table (bookingTable), since both share
; the same unified Invoice ID counter. Shows full detailed info
; (add-on cart items, or booking sport/court/total) for whichever
; table the ID is found in.
; =========================================================
...
SEARCH_AND_VIEW_INVOICE PROC
    ...
    ; BUGFIX: previously only checked dbRecordCount, so if only
    ; venue bookings existed (bookingRecordCount > 0) it wrongly
    ; reported "not found" without even asking for an ID.
    CMP dbRecordCount, 0
    JNE SIV_HAS_DATA
    CMP bookingRecordCount, 0
    JNE SIV_HAS_DATA
...
SIV_CHECK_SHORT:
    ...
    ; -------- Phase 1: search add-on invoice table (dbTable) --------
    CMP dbRecordCount, 0
    JE SIV_SEARCH_BOOKING
...
SIV_DB_FAIL:
    ...
    ; SI is unchanged here (restored on every failed attempt above),
    ; so it still points at the start of the 3-digit ID - safe to
    ; reuse directly for the booking-table search below.

SIV_SEARCH_BOOKING:
    CMP bookingRecordCount, 0
    JE INVOICE_NOT_FOUND

    MOV CX, bookingRecordCount
    XOR BX, BX
...
SIV_BK_LOOP:
    ...
    MUL CX
    LEA DI, bookingTable
    ADD DI, AX             ; Invoice ID digits stored at offset 0
...
SIV_FOUND_BOOKING:
    LEA DX, foundBookingMsg
    MOV AH, 09H
    INT 21H

    CALL PRINT_BOOKING_INVOICE_BY_INDEX
    JMP EXIT_SEARCH_INVOICE
...
DB_CART_PRICE:
    ...
    ; BUGFIX: the line used to stop after the unit price, so the
    ; quantity the admin actually picked never appeared anywhere
    ; on the printed invoice - now show " x <qty> = RM <subtotal>"
    ; using the values that ADD_TO_CART already stored per item.
    LEA DX, invQtyMsg
    MOV AH, 09H
    INT 21H

    XOR AX, AX
    MOV AL, [DI + 27]     ; Quantity
    CALL PRINT_NUMBER
...
; =========================================================
; HELPER: PRINT VENUE BOOKING INVOICE FROM bookingTable BY INDEX
; Used by "Search Invoice Record" when the ID belongs to a court
; booking rather than an add-on/equipment purchase.
; =========================================================

PRINT_BOOKING_INVOICE_BY_INDEX PROC
    PUSH AX
    PUSH BX
    PUSH CX
    PUSH DX
    ...
    LEA DX, detailedBookingHeader
    MOV AH, 09H
    INT 21H

    LEA DX, detBkInvoiceLbl
    MOV AH, 09H
    INT 21H
    LEA DX, [SI]           ; Invoice ID digits + '$'
    MOV AH, 09H
    INT 21H

    LEA DX, detBkBookingLbl
    MOV AH, 09H
    INT 21H
    LEA DX, [SI + 10]      ; Booking ID digits + '$'
    MOV AH, 09H
    INT 21H

    LEA DX, detBkSportLbl
    MOV AH, 09H
    INT 21H
    LEA DX, [SI + 20]      ; Sport name + '$'
    MOV AH, 09H
    INT 21H

    LEA DX, detBkCourtLbl
    MOV AH, 09H
    INT 21H
    LEA DX, [SI + 45]      ; Court number + '$'
    MOV AH, 09H
    INT 21H

    LEA DX, detBkTotalLbl
    MOV AH, 09H
    INT 21H
    MOV AX, [SI + 90]      ; Total, stored in cents
    CALL PRINT_MONEY
    ...
    POP AX
    RET
PRINT_BOOKING_INVOICE_BY_INDEX ENDP
...
ASK_COURT_ID:
    ...
    ; ---------------------------------------------
    ; VALIDATION: Court ID must be non-empty and
    ; contain digits only (previously ANY input,
    ; including empty input, was silently accepted).
    ; ---------------------------------------------
...
ITEM_SELECTED:
    ...
    ; ---------------------------------------------
    ; Ask quantity - BUGFIX:
    ; The old code read a SINGLE keystroke (INT 21H, AH=01H) for
    ; quantity. If the admin typed a multi-digit number (e.g. "12"),
    ; only the first digit was used as the quantity, and the leftover
    ; digit(s) + Enter stayed queued in the keyboard buffer. Those
    ; stray characters were then silently consumed by the NEXT
    ; single-key prompt (e.g. "Add another item? / Checkout?"),
    ; causing the flow to jump unexpectedly and making it look like
    ; entering a large quantity "throws an error". It also meant the
    ; quantity actually stored/used never matched what was typed.
    ;
    ; Fix: read the whole line with buffered input (AH=0AH) and
    ; parse it as a proper decimal number (1-99), so multi-digit
    ; quantities are read correctly and no stray keystrokes remain.
    ; ---------------------------------------------
...
ASK_QTY_RETRY:
    ...
    MOV CL, qtyInputBuf + 1
    MOV CH, 0
    CMP CX, 0
    JE INVALID_QUANTITY

    LEA SI, qtyInputBuf + 2
...
DISPLAY_DATABASE_RECORDS PROC
    ...
    ; BUGFIX: this used to check ONLY dbRecordCount (the Add-on/
    ; equipment cart table). Venue court bookings are saved into a
    ; completely SEPARATE table (bookingTable / bookingRecordCount)
    ; by SAVE_BOOKING_RECORD, so a court booking's invoice was never
    ; counted here - "View All Invoice Records" reported "No dynamic
    ; invoice records found!" even right after a successful court
    ; booking, because dbRecordCount was still 0. Now both counters
    ; are checked before declaring "no records".
    CMP dbRecordCount, 0
    JNE HAS_RECORDS
    CMP bookingRecordCount, 0
    JNE HAS_RECORDS
...
HAS_RECORDS:
    ...
    ; ---------------------------------------------
    ; SECTION 1: Add-on / equipment invoices (dbTable)
    ; ---------------------------------------------

    CMP dbRecordCount, 0
    JE SKIP_ADDON_SECTION
...
SKIP_ADDON_SECTION:
    ...
    ; ---------------------------------------------
    ; SECTION 2: Venue (court) booking invoices (bookingTable)
    ; ---------------------------------------------

    CMP bookingRecordCount, 0
    JE EXIT_VIEW_DB

    LEA DX, msgBookingViewHeader
    MOV AH, 09H
    INT 21H
...
PRINT_BOOKING_LOOP:
    ...
    MOV AX, [SI + 90]          ; Total - stored in CENTS by CALC_FEES,
    CALL PRINT_MONEY            ; so use PRINT_MONEY (X.YY), not PRINT_NUMBER
...
PRINT_CART_PRICE:
    ...
    ; BUGFIX: same as PRINT_INVOICE_BY_INDEX - show the quantity and
    ; the resulting subtotal for this item, not just the unit price.
    LEA DX, invQtyMsg
    MOV AH, 09H
    INT 21H

    XOR AX, AX
    MOV AL, [SI + 27]      ; Quantity
    CALL PRINT_NUMBER
